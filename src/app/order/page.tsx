  ...
  return (
    <div className="pb-24">
      ...
      <div className="mx-auto grid max-w-6xl gap-10 px-5 sm:px-8 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="border border-bean/15 bg-bean p-6 text-tan sm:p-8">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="font-serif text-3xl">Bag</h2>
            <button
              type="button"
              className="text-xs uppercase tracking-[0.22em] text-tan/75"
          ...
          {cart.length === 0 ? (
            <p className="text-sm leading-7 text-tan/70">
              Nothing here yet.{" "}
              <Link href="/menu" className="text-tan underline-offset-4 hover:underline">
                Open the menu
              </Link>{" "}
              and add a drink.
            </p>
              ...
              {cart.map((item) => (
                <li key={`${item.id}-${item.note ?? ""}`} className="flex items-center justify-between gap-4 py-4">
                  <div>
                    ...
                    <p className="text-sm text-tan/80">{formatPkr(item.price)}</p>
                  </div>
          ...
          <div className="mt-6 flex items-center justify-between border-t border-tan/15 pt-4">
            <span className="text-tan/70">Total</span>
            <span className="font-serif text-2xl text-tan">{formatPkr(total)}</span>
          </div>
        </section>

        <form onSubmit={onSubmit} className="space-y-5 border border-bean/15 bg-bean p-6 text-tan sm:p-8">
            ...
            <div className="grid grid-cols-2 gap-2">
              {(["pickup", "dine-in"] as const).map((option) => (
                <button
                  ...
                  className={
                    fulfillment === option
                      ? "rounded-lg border border-tan bg-tan/15 px-3 py-2 text-sm"
                      : "rounded-lg border border-tan/25 px-3 py-2 text-sm text-tan/70"
                  }
                >
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              className="min-h-24 border-tan/25 bg-transparent"
            />
          </div>
          <Button
            type="submit"
