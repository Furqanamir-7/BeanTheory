...
export function BookingForm({ event }: { event: CafeEvent }) {
  ...
  return (
    <form onSubmit={onSubmit} className="h-fit space-y-5 border border-bean/15 bg-bean p-6 text-tan sm:p-8">
      <h2 className="font-serif text-3xl">Book a slot</h2>
      <p className="text-sm text-tan/85">{formatPkr(event.price)} per guest</p>
      <div>
        ...
        <div className="grid gap-2">
          {event.slots.map((row) => (
            <button
              ...
              className={
                slotId === row.id
                  ? "flex items-center justify-between rounded-lg border border-tan bg-tan/15 px-3 py-2 text-sm"
                  : "flex items-center justify-between rounded-lg border border-tan/25 px-3 py-2 text-sm text-tan/70"
              }
            >
              <span>{row.time}</span>
              <span className="text-xs text-tan/50">{row.capacity} seats</span>
            </button>
          ))}
        </div>
      </div>
      <div className="space-y-2">
        ...
        <Input id="name" name="name" required className="h-10 border-tan/25 bg-transparent" />
      </div>
      <div className="space-y-2">
        <Label htmlFor="phone">Phone</Label>
        <Input id="phone" name="phone" required className="h-10 border-tan/25 bg-transparent" />
      </div>
      <div className="space-y-2">
        <Label>Guests</Label>
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm text-tan/55">{formatPkr(event.price * guests)} total</span>
        </div>
      </div>
      <div className="space-y-2">
        ...
        <Textarea
          ...
          className="min-h-20 border-tan/25 bg-transparent"
        />
      </div>
